...
export default (state = nameInitialState, action) => {
  switch (action.type) {
    ...
    case PD_FETCH_TRIP_INFO_SUCCESS: {
      ...
      const { driverName, createdByName, createdByPhone, tripCode, stopPoints } = info;
    ...
    case PDLIST_FETCH_SUCCESS: {
      ...
      if (more === false) {
        ...
        const nowTime = currentServerTime ? new Date(currentServerTime) : new Date();
        nowTime.setMinutes(nowTime.getMinutes() - 1);
        const lastUpdatedTime = nowTime.toISOString();
  ...
  // ]
    ...
    case PD_GET_ORDERS_INFO_SUCCESS: {
      const { data } = action.payload;
